    ...
    <div className={`${openMenu ? 'flex' : 'hidden'} w-full h-auto lg:w-1/5 m-1 p-2 shadow rounded-md lg:flex flex-col justify-between`}>
      <div className='sidebarTop'>
        ...
        <div className='my-5 flex flex-col gap-2'>
          {teams.map((team) => (
            <Tab key={team.team} id={team.team} onClick={() => handleTeamClick(team.team)} className={`p-2 flex items-center gap-2 justify-start hover:cursor-pointer ${highlightedTeam === team.team ? 'bg-gray-100' : ''}`}>
              <h4>{team.team}</h4>
            </Tab>
          ))}
            ...
            team.team === highlightedTeam && (
              team.folders.map((folder) => (
                <div key={`${team.team}-${folder.name}`}>
                  ...
                  <Tab key={folder.name} id={folder.name} onClick={() => handleFolderClick(folder.name)} className={`p-2 h-auto flex flex-wrap items-center gap-2 justify-start hover:cursor-pointer ${highlightedFolder === folder.name ? 'bg-gray-100' : ''}`}>
